...
def preprocess_image(image):
    #swap color channels, resize, add a batch dimension
...
def generate_adversaries(model,baseImage,delta,classIdx,steps=50):
    ...
    for step in range(0,steps):
        ...
        with tf.GradientTape() as tape:
            #explicitly indicate perturbation vector should be tracked for gradient updates
            tape.watch(delta)

            #add out perturbation vector to the base image and preprocess
            adversary = preprocess_input(baseImage + delta)

            #run this newly constructed image tensor through our model and calculate loss with respsect to original class index
        ...
        #update the weights, clip the perturbation bctor, and update its value
...
#generate the perturbation bector to create an adversarial example
...
#run inference with this adversarial example, parse the results and display
...
#draw the top-most preidcted on the adversarial along with confidence score
